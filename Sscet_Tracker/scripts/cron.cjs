...
async function runCron() {
  ...
  try {
    // 1. Get settings
    const { data: timeData } = await supabase.from('global_settings').select('value').eq('id', 'auto_email_time').single();
    const { data: activeData } = await supabase.from('global_settings').select('value').eq('id', 'auto_email_active').single();
    const { data: lastSentData } = await supabase.from('global_settings').select('value').eq('id', 'last_auto_email_date').single();
    ...
    console.log(`Current IST Time: ${currentTime}, Scheduled: ${scheduledTime}, Last Sent: ${lastSentDate}`);
    ...
    console.log("Time matched! Proceeding to fetch students and check LeetCode...");
    ...
    const studentsWithLc = students.filter((s) => s.leetCodeUsername && s.leetCodeUsername.trim() !== "");
    ...
    for (const student of studentsWithLc) {
      ...
      try {
        const res = await fetch(`https://alfa-leetcode-api.onrender.com/${clean}/acSubmission?limit=10`);
        if (res.ok) {
          ...
          const apiHasSolved = submissions.some((sub) => {
            ...
            const subDate = new Date((Number(sub.timestamp) * 1000) + istOffset).toISOString().split('T')[0];
          ...
          const fbRes = await fetch(`https://leetcode-api-faisalshohag.vercel.app/${clean}`);
          if (fbRes.ok) {
    ...
    console.log(`Found ${slackingStudents.length} inactive students with emails.`);

    // 4. Send Emails via EmailJS REST API
    if (slackingStudents.length > 0 && EMAILJS_SERVICE && EMAILJS_TEMPLATE && EMAILJS_PUBLIC_KEY) {
      let sentCount = 0;
    ...
    await supabase.from('global_settings').upsert({ id: 'last_auto_email_date', value: todayStr }, { onConflict: 'id' });
