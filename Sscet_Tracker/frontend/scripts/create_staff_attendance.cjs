const fs = require('fs');
const path = 'c:/Users/idhay/Downloads/Leetcode_Tracker/Sscet_Tracker/frontend/src/app/components/StaffAttendance.tsx';
  ...
  const loadStudents = async () => {
    try {
      ...
      const mapped = data.slice(0, 35).map((s: any) => {
        // Logic: If solved > 0 today, present = true
        const solvedToday = Math.floor(Math.random() * 3); // random 0, 1, 2
        return {
          ...s,
          solvedToday,
          isPresent: solvedToday > 0,
        };
      });
      setStudents(mapped);
    } catch (e) {
...
console.log("Created StaffAttendance.tsx");
