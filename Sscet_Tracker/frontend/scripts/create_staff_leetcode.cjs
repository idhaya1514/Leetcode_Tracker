  ...
  return (
    <div className="max-w-[1400px] mx-auto space-y-6 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-2">
        ...
        <button onClick={() => setIsConfiguring(!isConfiguring)} className="px-4 py-2 bg-white border border-slate-200 text-slate-700 text-sm font-medium rounded-lg hover:bg-slate-50 transition-colors shadow-sm flex items-center gap-2">
          <Settings2 className="w-4 h-4" /> Configure Today's Target
        </button>
      </div>

      {/* Target Configuration Panel */}
      {isConfiguring && (
        <div className="bg-slate-900 rounded-xl p-6 shadow-lg animate-in slide-in-from-top-4 duration-300">
          <div className="flex items-center gap-2 mb-4">
            <Target className="w-5 h-5 text-indigo-400" />
            <h2 className="text-lg font-semibold text-white">Set Today's LeetCode Target</h2>
          </div>
          <div className="grid grid-cols-3 gap-6 mb-6">
            <div>
              <label className="block text-xs font-semibold text-emerald-400 uppercase tracking-wider mb-2">Easy Problems</label>
              <input type="number" min="0" value={tempTarget.easy} onChange={(e) => setTempTarget({...tempTarget, easy: parseInt(e.target.value) || 0})} className="w-full px-4 py-2 bg-slate-800 border border-slate-700 rounded-lg text-white outline-none focus:border-indigo-500" />
            </div>
            <div>
              <label className="block text-xs font-semibold text-orange-400 uppercase tracking-wider mb-2">Medium Problems</label>
              <input type="number" min="0" value={tempTarget.medium} onChange={(e) => setTempTarget({...tempTarget, medium: parseInt(e.target.value) || 0})} className="w-full px-4 py-2 bg-slate-800 border border-slate-700 rounded-lg text-white outline-none focus:border-indigo-500" />
            </div>
            <div>
              <label className="block text-xs font-semibold text-rose-400 uppercase tracking-wider mb-2">Hard Problems</label>
              <input type="number" min="0" value={tempTarget.hard} onChange={(e) => setTempTarget({...tempTarget, hard: parseInt(e.target.value) || 0})} className="w-full px-4 py-2 bg-slate-800 border border-slate-700 rounded-lg text-white outline-none focus:border-indigo-500" />
            </div>
          </div>
          <div className="flex justify-end gap-3">
            <button onClick={() => setIsConfiguring(false)} className="px-4 py-2 bg-slate-800 text-slate-300 rounded-lg font-medium hover:bg-slate-700 transition-colors">Cancel</button>
            <button onClick={saveTarget} className="px-4 py-2 bg-indigo-600 text-white rounded-lg font-medium hover:bg-indigo-500 transition-colors">Apply Target to All</button>
          </div>
        </div>
      )}
...
console.log("Created StaffLeetCode.tsx");
