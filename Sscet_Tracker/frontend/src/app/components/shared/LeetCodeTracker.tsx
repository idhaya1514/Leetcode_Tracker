...
export default function LeetCodeTracker({
  ...
  const handleToggleScheduler = async () => {
    ...
    toast.success(
      newState ? "Cloud scheduler activated!" : "Cloud scheduler stopped.",
    );
  };
  ...
  const loadLeetCodeData = async () => {
    ...
    try {
      ...
      let studentsWithLc = allStudents.filter((s) => s.leetCodeUsername);

      // If used inside Staff Dashboard, filter only assigned students
      if (assignedRegNumbers && assignedRegNumbers.length > 0) {
        studentsWithLc = studentsWithLc.filter((s) =>
      ...
      // Optimize Leetcode data fetching by chunking (batching) requests
      const CHUNK_SIZE = 5;
      for (let i = 0; i < studentsWithLc.length; i += CHUNK_SIZE) {
        const chunk = studentsWithLc.slice(i, i + CHUNK_SIZE);

        await Promise.all(
          chunk.map(async (student) => {
            try {
              const BASE = "https://alfa-leetcode-api.onrender.com";
              const cleaned = encodeURIComponent(
                student.leetCodeUsername!.trim().replace(/^@/, ""),
              );

              const history = JSON.parse(
                localStorage.getItem("leetcode_attendance_history") || "{}",
              );
              const localDates: string[] =
                history[student.registerNumber] || [];
              ...
              try {
                ...
                if (cached) {
                  ...
                  if (
                    parsed &&
                    parsed.data &&
                    typeof parsed.data.totalSolved === "number"
                  ) {
              ...
              try {
                ...
                const [solvedRes, recentRes] = await Promise.all([
                  ...
                  fetch(
                    `${BASE}/${cleaned}/acSubmission?limit=10&t=${Date.now()}`,
                    fetchOpts,
                  ),
                ]);
                ...
                if (recentRes.ok) {
                  try {
                    ...
                    const submissions: any[] =
                      recentData.submission || recentData.data || [];
                ...
                console.warn(
                  `Primary API failed for ${cleaned}, trying fallback...`,
                );
                const fallbackRes = await fetch(
                  `https://leetcode-api-faisalshohag.vercel.app/${cleaned}?t=${Date.now()}`,
                  { cache: "no-store" },
                );
                if (fallbackRes.ok) {
                  ...
                  if (!fallbackData.errors) {
                    ...
                    const submissions: any[] =
                      fallbackData.recentSubmissions || [];
                    const apiHasSolved = submissions.some((sub: any) => {
                      if (!sub.timestamp) return false;
                      const subDate = new Date(
                        Number(sub.timestamp) * 1000,
  ...
  const executeSendReminders = async (isAutomated: boolean = false) => {
    ...
    const slackingStudentsWithEmail = progressData.filter(
      (p) => !p.hasSolvedToday && p.student.email,
    );
    const slackingStudentsWithoutEmail = progressData.filter(
      (p) => !p.hasSolvedToday && !p.student.email,
    );
    ...
    if (slackingStudentsWithEmail.length === 0) {
      if (!isAutomated) {
        if (slackingStudentsWithoutEmail.length > 0) {
          toast.warning(
            `${slackingStudentsWithoutEmail.length} inactive students have no email linked. Cannot send reminders.`,
          );
        } else {
    ...
    if (!isAutomated) {
      ...
      toast.info(
        `Automated Scheduler: Initiating background email dispatch to ${slackingStudentsWithEmail.length} inactive students...`,
      );
    }
...
Academic Administration`,
        ...
        window.alert(
          `EmailJS Error(Student: ${data.student.name}): \n\n${errMsg} `,
        );
        failCount++;
      }
    }
  ...
  return (
    <div
      className="rounded-2xl p-6 mt-6 backdrop-blur-xl animate-fade-in"
      style={{
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 mb-8">
        <div>
          ...
          <p className="text-sm mt-1" style={{ color: "#475569" }}>
            Track which students have solved at least one problem today.
          </p>
        </div>
      ...
      <div className="mb-6 p-4 rounded-xl flex flex-wrap items-center gap-4 bg-orange-50/50 border border-orange-100">
        ...
        {isSchedulerActive && (
          <p className="text-xs text-orange-600 w-full mt-1">
            <span className="font-bold">✅ Cloud Scheduler Active:</span> Emails
            will be dispatched automatically at {scheduledTime} by the GitHub
            Actions background engine, even if this browser tab is closed or
      ...
      {loading ? (
        ...
        <div className="text-center py-12">
          ...
          <p className="font-medium text-slate-550">
            No students have linked their LeetCode accounts yet.
          </p>
        </div>
      ) : (
              ...
              {progressData.map((data, idx) => (
                ...
                >
                  ...
                  <td className="px-4 py-3 text-sm">
                    <a
                      href={getLeetCodeProfileUrl(
                        data.student.leetCodeUsername,
                      )}
                      target="_blank"
                      rel="noreferrer"
                  ...
                  <td className="px-4 py-3 text-center">
                    {data.stats ? (
                      ...
                      <span
                        ...
                        title={
                          data.error || "Could not fetch stats from LeetCode"
                        }
                      >
                        Error fetching ⓘ
                      </span>
