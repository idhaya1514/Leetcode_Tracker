  ...
  const loadStudents = async () => {
    try {
      ...
      const mapped = data.map((s: any) => {
        ...
        // Mocking some historical attendance since we don't have historical data in the API yet
        // In a real app we'd fetch this from the backend
        const attendance = 100;
        
        const tasksAssigned = s.assignments?.length || 0;
        const tasksCompleted = s.assignmentCompletions?.length || 0;
  ...
  const handleDownload = (format: 'pdf' | 'excel') => {
    ...
    try {
      const exportData = students.map(s => ({
        ...
        "Tasks Assigned": s.stats.tasksAssigned,
        "Tasks Completed": s.stats.tasksCompleted,
        "Total Solved": s.stats.totalSolved,
        "Attendance %": s.stats.attendance,
      }));

      const filename = `Staff_Report_${reportType}_${new Date().toISOString().split('T')[0]}`;
      ...
      if (format === 'excel') {
        ...
        const doc = new jsPDF();
        doc.text(`Staff Report - ${reportType.toUpperCase()}`, 14, 15);
        autoTable(doc, {
          head: [['Student Name', 'Register No', 'Tasks Assigned', 'Tasks Completed', 'Total Solved', 'Attendance %']],
          body: students.map(s => [
            s.name, 
            s.registerNumber, 
            s.stats.tasksAssigned, 
