...
import { getStaffStudentsDetails, API_BASE_URL } from "../../services/api";
...
export default function StaffEmails() {
  ...
  const loadStudents = async () => {
    try {
      ...
      // Map completion dynamically based on if they have completed assignments today
      const mapped = data.map((s: any) => {
        const completedToday = s.assignmentCompletions?.length > 0;
        return {
          ...s,
  ...
  const handleBulkReminders = async () => {
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/email/remind`, { method: 'POST' });
  ...
  const handleBulkCongrats = async () => {
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/email/congratulate`, { method: 'POST' });
      const data = await res.json();
      if (res.ok) {
