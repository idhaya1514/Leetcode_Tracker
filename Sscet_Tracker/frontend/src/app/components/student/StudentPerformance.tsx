...
export default function StudentPerformance({ onBack }: StudentPerformanceProps) {
  ...
  const loadData = async () => {
    setIsLoading(true);
    try {
      const data = await getAnalytics(selectedDept, selectedYear, 'All');
  ...
  const linkedProfiles = deptRegistrationData.reduce((acc: number, curr: any) => acc + curr.linked, 0);

  // Leaderboard placeholder (Requires complex query for attendance mapping)
          ...
          <div className="bg-white rounded-xl border border-slate-200/60 p-5 shadow-[0_2px_10px_-3px_rgba(6,81,237,0.05)] transition-all duration-200 hover:shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:border-slate-300">
            <div className="flex items-center gap-4 mb-4">
              ...
              <div>
                ...
                <h2 className="text-2xl font-semibold tracking-tight text-orange-600">0</h2>
              </div>
            </div>
          </div>
        </div>
